...
```{r m7_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
```{r m9_condPlot, anchor=FIGREF, fig.width=12, fig.height=12}
...
`r figr("m9_condPlot", type = FIGREF)`. *Conditional plots.*
...
Data wrangling and analyses were carried out with the statistical package `R` [`r version$version.string`; @RCT2020]. Bayesian modeling was done with the package `brms` [@Buerkner2017;@Buerkner2018] which uses the probabilistic language Stan as back end [@Carpenter2017]. Plots were done with the packages `bayesplot` [@Gabry2020] and `ggplot2` [@Wickham3016].
