...
It is obvious that some participants do not differ substantially between the normal and the controlled condition, whereas other do, and markedly so. Additionally, there seems to be quite a spread of the centers of distributions across a wide range of values, suggesting very individual drum sounds.

The wide distribtion of centers of mass between individuals made me want to further break down the plot.

```{r DensIndivRespPatientsPlot, ancho=FIGREF}
ggplot(drum_beats, aes(transSC, 
                       color = subj, 
                       linetype = cond)) + 
  geom_density(alpha = 0.1) +
...
`r figr("DensIndivRespPatientsPlot", type = FIGREF)`. *`transSC` density distribution of participants, broken down by instruction condition and group (patients and healthy subjects).*

***

The above plot is interesting in that it seems to show that the four healthy subjects were more uniform than the patients in their `transSC` distributions and also, with the exception of S2, had very similar `transSC` distributions for the normal and the controlled conditions. In the patients, two had very similar distributions in both conditions (P1 and P5), whereas the two others showed differing results for the two conditions. So with regard to Sofia's hypothesis (ch. [Modeling]) I'd argue, at least for `transSC` alone, having drummers play normal and controlled strokes would not allow subjects to tell the difference. But maybe it would qualify as a screening test for movement disorders in drummers. Just a random thought.

While breaking it down it occurred to me that looking at individual variation might also be enlightening.
...
And it was! The plot made visible that P1 and P5 had consistently very low transSC values. P3 was consistent within the normal instruction condition, but had, on average, higher values, and with a lot more variation, in the controlled condition. P4 had more variation in both conditions, and higher transSC values in both; in other words, P4 was consistantly bad.
...
```{r}
(m3_prior <- c(set_prior("normal(0, 1)", class = "sd")))
```
...
### Condition x Arm Model


```{r}
(m4_form <-bf(transSC ~ cond * arm +
                (1 | subj) + 
                (1 | cond) + 
                (1 | arm)))
```
...
In the Population-Level Effects section of `r figr("m4_summary")` are now two estimates for the skewness parameter alpha (one for the controlled [`alpha_Intercept`] and one for the normal condition [`alpha_condN`]), while there is no entry for alpha in the Family Specific section anymore. All this due to our explicit modeling of alpha conditional on instruction condition.
...
```{r, fig.width=12}
wrap_plots(plot(conditional_effects(m4),
             plot = FALSE)) +
...
```{r m5_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
p <- plot(conditional_effects(m5,
                              dpar = "mu"),
          plot = FALSE)
p1 <- wrap_plots(p[[1]]+
                   p[[2]] +
...
`r figr("m5_condPlot", type = FIGREF)`. *Conditional plot.*

***

In `r figr("m5_condPlot")` we see a difference between the mean ('mu') estimated value for `transSC` depending on instruction condition (A), but no difference between dominant and non-dominant arm (B). Consequently, there is no interaction in (C).
...
```{r m6_condPlot, anchor=FIGREF, fig.width=12, fig.height=12}
p <- plot(conditional_effects(m6,
                              dpar = "mu"),
          plot = FALSE)
...
In the Population-Level Effects section of `r figr("m7_summary")` are now three estimates for the skewness parameter alpha, as well as three estimates for spread ('sigma').
...
```{r}
loo_compare(m0, m1, m2, m3, m4, m5, m6, m7)
```

The model with the smallest `elpd` is `m7`, which corresponds to the last model. 

### Conclusion

Each row of the plot in `r figr("m7_condPlot")` represents the results of one of the three outcomes, each column shows the estimates for one condition/manipulation. The dots in these plots are the means of the posterior distributions of the respective estimate, and can be interpreted similarly to the point estimates in traditional frequentist statistics. The error bars represent 95% credible intervals, which are interpreted as comprising the true value with 95% probability, given the model and the data. 

From the first row of `r figr("m7_condPlot")` we can infer that instructions had an effect on the mean, while arm had not. The second row suggests that instruction had a larger, and opposing, effect on skewness compared to arm arm, and finally, the third row claims the same for spread.
...
```{r}
(m8_form <-bf(mvbind(transSC, transFlat) ~ cond + arm + 
                (1|p|cond) +
                (1|q|arm) + 
                (1|r|subj),
              sigma ~ cond + arm,
...
# 8                                   b     armND       transFlat                  
# 9                                   b     condN       transFlat                  
# 10   student_t(3, 0.8, 2.5) Intercept                 transFlat                  
# 11     student_t(3, 0, 2.5)        sd                 transFlat                  
# 12                                 sd             arm transFlat                  
# 13                                 sd Intercept   arm transFlat                  
# 14                                 sd            cond transFlat                  
# 15                                 sd Intercept  cond transFlat                  
# 16                                 sd            subj transFlat                  
# 17                                 sd Intercept  subj transFlat                  
# 18                                  b                 transFlat alpha            
# 19                                  b     armND       transFlat alpha            
# 20                                  b     condN       transFlat alpha            
# 21             normal(0, 4) Intercept                 transFlat alpha            
# 22                                  b                 transFlat sigma            
# 23                                  b     armND       transFlat sigma            
# 24                                  b     condN       transFlat sigma            
# 25     student_t(3, 0, 2.5) Intercept                 transFlat sigma            
# 26                                  b                   transSC                  
# 27                                  b     armND         transSC                  
# 28                                  b     condN         transSC                  
# 29 student_t(3, 735, 136.5) Intercept                   transSC                  
# 30   student_t(3, 0, 136.5)        sd                   transSC                  
# 31                                 sd             arm   transSC                  
# 32                                 sd Intercept   arm   transSC                  
# 33                                 sd            cond   transSC                  
# 34                                 sd Intercept  cond   transSC                  
# 35                                 sd            subj   transSC                  
# 36                                 sd Intercept  subj   transSC                  
# 37                                  b                   transSC alpha            
# 38                                  b     armND         transSC alpha            
# 39                                  b     condN         transSC alpha            
# 40             normal(0, 4) Intercept                   transSC alpha            
# 41                                  b                   transSC sigma            
# 42                                  b     armND         transSC sigma            
# 43                                  b     condN         transSC sigma            
# 44     student_t(3, 0, 2.5) Intercept                   transSC sigma 
(m8_prior <- c(
  set_prior("normal(0.8, 0.01)", class = "Intercept", resp = "transFlat"),
  set_prior("normal(0, 0.01)",   class = "b",         resp = "transFlat"),
  set_prior("normal(0.8, 1)",   class = "Intercept", resp = "transSC"),
  set_prior("normal(0, 1)",     class = "b",         resp = "transSC"),
  set_prior("normal(0, 0.01)",   class = "Intercept", resp = "transFlat", dpar = "alpha"),
  set_prior("normal(0, 0.01)",   class = "b",         resp = "transFlat", dpar = "alpha"),
  set_prior("normal(0, 0.1)",   class = "Intercept", resp = "transSC",   dpar = "alpha"),
  set_prior("normal(0, 0.1)",   class = "b",         resp = "transSC",   dpar = "alpha"),
  set_prior("normal(0, 0.05)",   class = "Intercept", resp = "transSC",   dpar = "sigma"),
  set_prior("normal(0, 0.05)",   class = "b",         resp = "transSC",   dpar = "sigma"),
  set_prior("normal(0, 0.01)",   class = "Intercept", resp = "transFlat", dpar = "sigma"),
  set_prior("normal(0, 0.01)",   class = "b",         resp = "transFlat", dpar = "sigma"),
  set_prior("normal(0, 0.05)",   class = "sd",        resp = "transSC"),
  set_prior("normal(0, 0.01)",  class = "sd",        resp = "transFlat"),
  set_prior("lkj(5)",           class = "cor")))
```
...
In the Population-Level Effects section of `r figr("m8_summary")` are now three estimates for the skewness parameter alpha, as well as three estimates for spread ('sigma').

```{r m8_ppcheck, anchor=FIGREF}
pp_check(m8, resp = "transSC", nsamples = 100)
pp_check(m8, resp = "transFlat", nsamples = 100)
```
...
```{r m8_condPlot, anchor=FIGREF, fig.width=12, fig.height=12}
