...
As is apparent from `r figr("DescriptorDensities")`, the controlled condition yielded broader density distributions in most descriptors. Hence, we model this condition-dependent variation in the next model:

```{r}
(m3_form <-bf(transSC ~ cond + 
                (1 | subj) + 
                (1 | cond),
              sigma ~ cond))
```
...
In the Links section of the summary (`r figr("m3_summary")`) we note that sigma is no longer modeled on the identity scale but on the $\log_{2}$ scale. In the Population-Level Effects section of `r figr("m3_summary")` are now two estimates for sigma (one for the controlled [`sigma_Intercept`] and one for the normal condition [`sigma_condN`]), while there is no entry for sigma in the Family Specific section. All this due to our explicit modeling of sigma conditional on instruction condition.
...
## Modeling Condition-specific Skewness

At least some descriptors in `r figr("DescriptorDensities")` the skewness of the distribution seems to change between conditions. Hence, we model this condition-dependent skewness in the next model:

```{r}
(m4_form <-bf(transSC ~ cond + 
                (1 | subj) + 
                (1 | cond),
              sigma ~ cond,
              alpha ~ cond))
...
```{r}
(m5_prior <- c(set_prior("normal(0, 1)", class = "sd"),
