...
`r figr("m10_condPlot", type = FIGREF)`. *Conditional plots. Had to take out all the fancy figure polishing, subplot lettering etc. Subplots are supposed to be numbered from A to ... starting in the top left corner, proceeding row-wise.* 
...
### Modeling the Influence of Group on Shape and Spread
...
```{r}
(m10_transFlat_form <-bf(transFlat ~ 0 + Intercept + patient * cond * arm + 
                           (1 | subj) + 
                           (1 | cond) +
                           (1 | arm),
                         sigma ~ cond * arm,
...
`r figr("m10_transFlat_summary", type = TABREF)`. *Model summary. The model has not converged.*

```{r m10_transFlat_summary, achor = TABREF}
(m10_transFlat_summary <- summary(m10_transFlat, 
                                  priors = TRUE))
```


```{r m10_transFlat_ppcheck, anchor=FIGREF}
pp_check(m10_transFlat, nsamples = 100)
```

`r figr("m10_transFlat_ppcheck", type = FIGREF)`. *Posterior predictive check. The thick blue line shows the distribution of the empirical data. The thin blue lines are one-hundred realizations of data generated from parameters estimated by the model.*

***

```{r m10_transFlat_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
`r figr("m10_transFlat_condPlot", type = FIGREF)`. *Conditional plots. Had to take out all the fancy figure polishing, subplot lettering etc. Subplots are supposed to be numbered from A to ... starting in the top left corner, proceeding row-wise.* 

***

In `r figr("m10_transFlat_condPlot")` the dots are the means of the posterior distributions of the respective estimate, and can be interpreted similarly to the point estimates in traditional frequentist statistics. The error bars represent 95% credible intervals, which are interpreted as comprising the true value with 95% probability, given the model and the data.
...
```{r Best_transFlatModel3wayInteractPlot, anchor=FIGREF, fig.width = 12}
...
```{r}
(m10_transCrest_form <-bf(transCrest ~ 0 + Intercept + patient * cond * arm + 
                            (1 | subj) + 
                            (1 | cond) +
                            (1 | arm),
                          sigma ~ cond * arm,
...
`r figr("m10_transCrest_summary", type = TABREF)`. *Model summary.*

```{r m10_transCrest_summary, achor = TABREF}
(m10_transCrest_summary <- summary(m10_transCrest, 
                                   priors = TRUE))
```


```{r m10_transCrest_ppcheck, anchor=FIGREF}
pp_check(m10_transCrest, nsamples = 100)
```

`r figr("m10_transCrest_ppcheck", type = FIGREF)`. *Posterior predictive check. The thick blue line shows the distribution of the empirical data. The thin blue lines are one-hundred realizations of data generated from parameters estimated by the model.*

***

```{r m10_transCrest_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
`r figr("m10_transCrest_condPlot", type = FIGREF)`. *Conditional plots. Had to take out all the fancy figure polishing, subplot lettering etc. Subplots are supposed to be numbered from A to ... starting in the top left corner, proceeding row-wise.* 

***

In `r figr("m10_transCrest_condPlot")` the dots are the means of the posterior distributions of the respective estimate, and can be interpreted similarly to the point estimates in traditional frequentist statistics. The error bars represent 95% credible intervals, which are interpreted as comprising the true value with 95% probability, given the model and the data.
...
```{r Best_transCrestModel3wayInteractPlot, anchor=FIGREF, fig.width = 12}
