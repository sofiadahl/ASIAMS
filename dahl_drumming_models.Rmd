...
```{r DensIndivRespPlot, anchor=FIGREF, fig.width=7}
...
```{r DensIndivRespGroupedPlot, ancho=FIGREF, fig.width=7}
ggplot(drum_beats, aes(transSC, 
                       color = subj, 
                       linetype = cond)) + 
  geom_density(alpha = 0.1) +
...
```{r DensIndivSeriesGroupedPlot, anchor=FIGREF, fig.width=12, fig.height=7}
ggplot(drum_beats, aes(transSC, 
                       color = cond, 
                       group = series)) + 
  geom_density(alpha = 0.1) +
...
```{r DensIndivRespPlot_transFlat, anchor=FIGREF, fig.width=7}
...
`r figr("DensIndivRespPlot_transFlat", type = FIGREF)`. *`transFlat` density distribution of participants, broken down by instruction condition.*

***

```{r DensIndivRespGrouped_transFlatPlot, ancho=FIGREF, fig.width=7}
ggplot(drum_beats, aes(transFlat, 
                       color = subj, 
                       linetype = cond)) + 
  geom_density(alpha = 0.1) +
...
`r figr("DensIndivRespGrouped_transFlatPlot", type = FIGREF)`. *`transFlat` density distribution of participants, broken down by instruction condition and group (patients and healthy subjects).*

***

```{r DensIndivSeriesGrouped_transFlatPlot, anchor=FIGREF, fig.width=12, fig.height=7}
ggplot(drum_beats, aes(transFlat, 
                       color = cond, 
                       group = series)) + 
  geom_density(alpha = 0.1) +
...
`r figr("DensIndivSeriesGrouped_transFlatPlot", type = FIGREF)`. *`transFlat` density plots of individual series.*

***

```{r DensIndivRespPlot_transCrest, anchor=FIGREF, fig.width=7}
...
`r figr("DensIndivRespPlot_transCrest", type = FIGREF)`. *`transCrest` density distribution of participants, broken down by instruction condition.*

***

```{r DensIndivRespGrouped_transCrestPlot, ancho=FIGREF, fig.width=7}
ggplot(drum_beats, aes(transCrest, 
                       color = subj, 
                       linetype = cond)) + 
  geom_density(alpha = 0.1) +
...
`r figr("DensIndivRespGrouped_transCrestPlot", type = FIGREF)`. *`transCrest` density distribution of participants, broken down by instruction condition and group (patients and healthy subjects).*

***

```{r DensIndivSeriesGrouped_transCrestPlot, anchor=FIGREF, fig.width=12, fig.height=7}
ggplot(drum_beats, aes(transCrest, 
                       color = cond, 
                       group = series)) + 
  geom_density(alpha = 0.1) +
...
`r figr("DensIndivSeriesGrouped_transCrestPlot", type = FIGREF)`. *`transCrest` density plots of individual series.*
...
```{r m6a_conditionalPlots, fig.width=12, fig.height=12}
...
```{r m7_condPlot, anchor=FIGREF, fig.width=12, fig.height=12}
...
```{r m8_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
```{r m9_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
### Modeling Interactions of Arm and Instruction on Shape and Spread
...
```{r m10_condPlot, anchor=FIGREF, fig.width=12, fig.height=12}
...
### The Full Model: Modeling the Interactions of Group, Instruction, and Side on Shape and Spread
...
`r figr("m11_summary", type = TABREF)`. *Model summary.*

```{r m11_summary, achor = TABREF}
(m11_summary <- summary(m11, 
                        priors = TRUE))
```


```{r m11_ppcheck, anchor=FIGREF}
pp_check(m11, nsamples = 100)
```

`r figr("m11_ppcheck", type = FIGREF)`. *Posterior predictive check. The thick blue line shows the distribution of the empirical data. The thin blue lines are one-hundred realizations of data generated from parameters estimated by the model.*

***

```{r m11_condPlot, anchor=FIGREF, fig.width=12, fig.height=12}
...
`r figr("m11_condPlot", type = FIGREF)`. *Conditional plots. Had to take out all the fancy figure polishing, subplot lettering etc. Subplots are supposed to be numbered from A to ... starting in the top left corner, proceeding row-wise.* 

***

In `r figr("m11_condPlot")` the dots are the means of the posterior distributions of the respective estimate, and can be interpreted similarly to the point estimates in traditional frequentist statistics. The error bars represent 95% credible intervals, which are interpreted as comprising the true value with 95% probability, given the model and the data.
...
The model with the smallest `elpd` is `m10`, which corresponds to the second to last model (see [Modeling Interactions of Arm and Instruction on Shape and Spread]). The difference between the best and the second-best model (m11, [Modeling the Influence of Group on Shape and Spread]) is `r round(univ_model_compar["m11", "elpd_diff"], 2)`, which is less than roughly two SE (2 x `r round(univ_model_compar["m11", "se_diff"], 2)` = `r round(2 * univ_model_compar["m11", "se_diff"], 2)`) away from 0. Therefore, we consider the best fitting and the second-best fitting models (roughly) equivalent. Based on theoretical grounds, modeling the three estimates `mu`, `sigma` and `alpha` should be done with the full model--*i.e.*, containing all the main effect interactions--if there is no risk of overfitting. Therefore, in the following we use the full model ([Modeling the Influence of Group on Shape and Spread]).
...
```{r BestModel3wayInteractPlot, anchor=FIGREF, fig.width=12, fig.height=6}
...
`r figr("BestModel3wayInteractPlot", type = FIGREF)`. *Three-way interaction plots of the three modeled parameters $\mu$, $\sigma$, and $\alpha$ as estimated by the full model. The x axis shows group membership (0 = healthy subjects, 1 = patients). The instruction conditions are color-coded (black = controlled strokes, brown = normal strokes), and the facets show the side (1 = dominant, 2 = non-dominant).*

***

`r figr("BestModel3wayInteractPlot")`A suggests that $\mu$ is higher for healthy subjects than for patients, and higher for controlled strokes than for normal strokes which is really odd on an intuitive level and seems contradictive (at least to me). I would've expected `transSC` to be at some optimal level in healthy subjects while performing normal strokes with their dominant side, and decrease from there. Yet it does not seem to be so simple. The biggest and clearest difference between any of the experimental factors is between instruction conditions "normal" and "controlled" (see `r figr("m11_summary")` for the credible intervals of these estimates). Of the interactions shown in `r figr("BestModel3wayInteractPlot")`A, only the estimate for `patient1:armND` is sufficiently far from zero, while the central 95% of the posterior distribution of `patient1:condN:armND` is, after all, located mostly to the left of zero.

`r figr("BestModel3wayInteractPlot")`B might clarify why this is the case, because it shows a higher unexplained variation in the data for controlled strokes as compared to normal strokes. And since higher variation is often accompanied by more extreme values, these extreme values might drive the mean of the posterior distribution of $\mu$ up. Interestingly, $\sigma$ is estimated to be almost the same low value across groups and sides in the normal instruction condition, whereas it is unanimously high in the controlled condition.

`r figr("BestModel3wayInteractPlot")`C gives the impression that in the normal condition the distribution of `transSC` roughly symmetrical around zero, in the controlled condition there is an increased amount of higher `transSC` values making the distribution heavy-tailed on the right side (see *e.g.*, [Wikipedia on skewness](https://en.wikipedia.org/wiki/Skewness#Introduction)).

Summarizing these results, `transSC` is, on average, lower in normal strokes while approximately normally distributed, and with lower spread than in controlled strokes. The fact that there is a difference between the two instructions suggests that this descriptor might serve as a cue in listening tests.

## Univariate Models -- `transFlat`
...
`r figr("m11_transFlat_summary", type = TABREF)`. *Model summary. The model has not converged.*

```{r m11_transFlat_summary, achor = TABREF}
(m11_transFlat_summary <- summary(m11_transFlat, 
                                  priors = TRUE))
```


```{r m11_transFlat_ppcheck, anchor=FIGREF}
pp_check(m11_transFlat, nsamples = 100)
```

`r figr("m11_transFlat_ppcheck", type = FIGREF)`. *Posterior predictive check. The thick blue line shows the distribution of the empirical data. The thin blue lines are one-hundred realizations of data generated from parameters estimated by the model.*

***

```{r m11_transFlat_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
`r figr("m11_transFlat_condPlot", type = FIGREF)`. *Conditional plots. Had to take out all the fancy figure polishing, subplot lettering etc. Subplots are supposed to be numbered from A to ... starting in the top left corner, proceeding row-wise.* 

***

### Conclusions

```{r Best_transFlatModel3wayInteractPlot, anchor=FIGREF, fig.width = 12, fig.height=6}
...
In `transFlat` there is a small but most likely positive difference between controls and patients, but all the other main effects are not clearly different from zero (`r figr("BestModel3wayInteractPlot")`A). Variation shows a curiously extreme estimated value for healthy subjects playing controlled strokes on the non-dominant side (`r figr("BestModel3wayInteractPlot")`B) which seems to be mostly driven by a relatively large estimated value for the non-dominant side (`sigma_armND` in `r figr("m11_transFlat_summary")`). `r figr("BestModel3wayInteractPlot")` shows that the left tail of `transFlat`'s distribution gets heavier in the controlled condition

Since there is no clear difference in the average `transFlat` values between conditions this descriptor is probably not a suitable cue to distinguish between conditions in a listening test.

## Univariate Models -- `transCrest`
...
`r figr("m11_transCrest_summary", type = TABREF)`. *Model summary.*

```{r m11_transCrest_summary, achor = TABREF}
(m11_transCrest_summary <- summary(m11_transCrest, 
                                   priors = TRUE))
```


```{r m11_transCrest_ppcheck, anchor=FIGREF}
pp_check(m11_transCrest, nsamples = 100)
```

`r figr("m11_transCrest_ppcheck", type = FIGREF)`. *Posterior predictive check. The thick blue line shows the distribution of the empirical data. The thin blue lines are one-hundred realizations of data generated from parameters estimated by the model.*

***

```{r m11_transCrest_condPlot, anchor=FIGREF, fig.width=12, fig.height=8}
...
`r figr("m11_transCrest_condPlot", type = FIGREF)`. *Conditional plots. Had to take out all the fancy figure polishing, subplot lettering etc. Subplots are supposed to be numbered from A to ... starting in the top left corner, proceeding row-wise.* 

***

### Conclusions

```{r Best_transCrestModel3wayInteractPlot, anchor=FIGREF, fig.width = 12, fig.height=6}
