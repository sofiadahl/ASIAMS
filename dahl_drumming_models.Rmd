...
`r figr("BestModel3wayInteractPlot")`C gives the impression that in the normal condition the distribution of `transSC` is roughly symmetrical around zero, while in the controlled condition there is an increased amount of higher `transSC` values making the distribution heavy-tailed on the right side (see *e.g.*, [Wikipedia on skewness](https://en.wikipedia.org/wiki/Skewness#Introduction)).
...
In `transFlat` there is a small but most likely positive difference between controls and patients, but all the other main effects are not clearly different from zero (`r figr("BestModel3wayInteractPlot")`A). 

Variation shows a curiously extreme estimated value for healthy subjects playing controlled strokes on the non-dominant side (`r figr("BestModel3wayInteractPlot")`B) which seems to be mostly driven by a relatively large estimated value for the non-dominant side (`sigma_armND` in `r figr("m11_transFlat_summary")`). 

`r figr("BestModel3wayInteractPlot")`C shows that the left tail of `transFlat`'s distribution gets heavier in the controlled condition, suggesting a tendency in players to produce a larger amount of low-valued `transFlat` values when performing unfamiliar strokes.
