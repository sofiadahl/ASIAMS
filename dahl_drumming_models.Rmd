...
The data set was read in as is, the columns `subj`, `arm`, `cond` and `no` were converted to factors, the columns `series` and `filename` were deleted.
...
Looking at @Danielsen2015 and the above investigated descriptors it seems reasonable to limit the modeling attempts to `totDur`, `totSPL`, `totSC`, and `TC`.
...
Looking again at the variables *I* have decided on to be the ones to be used as response variables in the regression models---implying that this is by no means set in stone---, seems like a skewed normal or lognormal link function would be appropriate to model them.

Let's start with totDur
