...
```{r}
univ_model_compar <- loo_compare(m0, m2, m3, m5, m6, m6a, m7, m8, m9, m10)
print(univ_model_compar, simplify = T)
```

The model with the smallest `elpd` is `m10`, which corresponds to the last model (see [Modeling Influence of Instruction and Arm on Shape]). The difference between the best and the second-best model is `r round(univ_model_compar["m9", "elpd_diff"], 2)`, which is more than roughly two SE (2 x `r round(univ_model_compar["m9", "se_diff"], 2)` = `r round(2 * univ_model_compar["m9", "se_diff"], 2)`) away from 0.
...
```{r}
(m12_form <-bf(mvbind(transSC, transFlat) ~ cond + arm + 
                 (1|p|cond) +
                 (1|q|arm) + 
                 (1|r|subj),
               sigma ~ cond + arm,
...
`r figr("m12_summary", type = TABREF)`. *Model summary.*

```{r m12_summary, achor = TABREF}
# (m12_summary <- summary(m12, 
#                        priors = TRUE))
```

In the Population-Level Effects section of `r figr("m12_summary")` are now three estimates for the skewness parameter alpha, as well as three estimates for spread ('sigma').

```{r m12_ppcheck, anchor=FIGREF}
# pp_check(m12, resp = "transSC", nsamples = 100)
# pp_check(m12, resp = "transFlat", nsamples = 100)
```

`r figr("m12_ppcheck", type = FIGREF)`. *Posterior predictive check. The thick blue line shows the distribution of the empirical data. The thin blue lines are one-hundred realizations of data generated from parameters estimated by the model.*

***

```{r m12_condPlot, anchor=FIGREF}
# , fig.width=12, fig.height=12
# p <- plot(conditional_effects(m12,
#                               dpar = "mu"),
#           plot = FALSE)
# p1 <- wrap_plots(p[[1]] + xlab("") +
#                    p[[2]] + xlab("") + ylab("") +
